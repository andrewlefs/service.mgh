...
class LevelRacingOpenBetaModel extends ModelObject implements ModelObjectInterface {
     ...
     * @param int $app
     */
    public function getLevelRacing(array $fields = array(), $cached = true) {
        ...
        if ($queryResult == false || $cached == false) {
            $query = $this->getConnection()->select(count($fields) == 0 ? '*' : implode(',', $fields))
                    ->order_by("level", "desc")
                    ->get(ModeTableNamelEnum::LEVEL_RACING_OPEN_BETA);
    ...
    public function getListLogs(array $keys, array $fields = array(), $cached = true) {
        ...
        if ($queryResult == false || $cached == false) {
            $query = $this->getConnection()->select(count($fields) == 0 ? '*' : implode(',', $fields))
                    ->where($keys)
                    ->order_by("id", "desc")
                    ->get(ModeTableNamelEnum::LEVEL_RACING_OPEN_BETA_LOGS);
    ...
    public function addLogs(array $data) {
        ...
        return parent::insert(ModeTableNamelEnum::LEVEL_RACING_OPEN_BETA_LOGS, $data);
    }

    public function updateLog(array $data, array $wheres) {
        $sql = $this->getConnection()->update(ModeTableNamelEnum::LEVEL_RACING_OPEN_BETA_LOGS, $data, $wheres);
        //var_dump($this->getConnection()->last_query());die;
        return $this->getConnection()->affected_rows();
    }

    public function updateData(array $data, array $wheres) {
        $sql = $this->getConnection()->update(ModeTableNamelEnum::LEVEL_RACING_OPEN_BETA, $data, $wheres);
        //var_dump($this->getConnection()->last_query());die;
        return $this->getConnection()->affected_rows();
    }

    public function getEndPoint() {
        return "LevelRacingOpenBetaModel";
    }
