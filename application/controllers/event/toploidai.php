...
class toploidai extends EI_Controller {
    ...
    public function index() {
        ...
        echo $this->load->view("events/toploidai/index", $this->data, true);
    }
    ...
    public function thamgia() {
        ...
        //Get Tournament List 
        $id = $_GET["id"];
        $tournament_list = $this->toploidai->get_tournament_details($id);
        ...
        $data["TopArena"] = $this->toploidai->Event_TopArena_GetList($server_id, 1);
        $data["TopBattlePoint"] = $this->toploidai->Event_TopBattlePoint_GetList($server_id, 1);
...
//        var_dump($data["TopBattlePoint"]); die;
        ...
        echo $this->load->view("events/toploidai/thamgia", $this->data, true);
    }
    ...
    public function get_pk_point_test() {
        ...
        //echo $Sever_ID; die;
        $data["user_point"] = $this->toploidai->get_pk_point_new($tournament_store_proc, $Sever_ID, $UserID);
    ...
    function get_exchange_history() {
        ...
        $data["history"] = $this->toploidai->get_exchange_history_new($tournament_id, $char_id, $server_id, $mobo_service_id);
        $this->data["history"] = $data["history"];

        $data["history_top"] = $this->toploidai->get_exchange_history_new_top($tournament_id, $char_id, $server_id, $mobo_service_id);
        $this->data["history_top"] = $data["history_top"];

        $data["history_top_premiership"] = $this->toploidai->get_exchange_history_premiership($tournament_id, $char_id, $server_id, $mobo_service_id);
        $this->data["history_top_premiership"] = $data["history_top_premiership"];

        $this->output->set_header("Cache-Control: no-store, no-cache, must-revalidate");
        echo $this->load->view("events/toploidai/history", $this->data, true);
    }
    ...
    public function gift_exchange() {
        if (isset($_SESSION["execute_time"]) && (time() - $_SESSION["execute_time"]) < 10) {
            ...
            //else{
            //Check Reward Valid
            $reward_details = $this->toploidai->get_reward_details($id);
            if (count($reward_details) > 0) {
                if (!$this->getSession($mobo_service_id, $server_id)) {
                    ...
                    //Check Exist            
                    if ($this->toploidai->check_exist_exchange_gift($reward_details[0]["tournament_id"], $server_id, $char_id)) {
                        $result["code"] = "-1";
                        $result["message"] = "Bạn đã nhận quà giải đấu này rồi !";
                    } else {
                        //Check Receive Gift Date
                        $get_tournament_details = $this->toploidai->get_tournament_details($reward_details[0]["tournament_id"]);
                        ...
                        if (strtotime($date_now) > strtotime($tournament_date_end_reward)) {
                            ...
                            $userpoint = $this->toploidai->get_pk_point_new($get_tournament_details[0]["tournament_store_proc"], $server_id_merge, $char_id);
                            ...
                            if ($userpoint[0]["CURRPOINT"] < $reward_details[0]["reward_point"] || $userpoint[0]["CURRPOINT"] == "") {
                                ...
                                $i_id = $this->toploidai->insert_id("event_toppk_exchange_history", $userdata_p);

                                if ($i_id > 0) {
                                    //Write Cache Log              
                                    $write_cache = $this->toploidai->write_cache_log($char_id, $server_id, $userpoint[0]["CURRPOINT"], $reward_details[0]["reward_point"], $id, Date('Y-m-d H:i:s'));
                                    ...
                                    if ($write_cache[0]["Errcode"] == "00000") {
                                        ...
                                        $this->toploidai->update_exchange_history($i_id, json_encode($item1), json_encode($data_result));
    ...
    public function gift_top_exchange() {
        if (isset($_SESSION["execute_time"]) && (time() - $_SESSION["execute_time"]) < 10) {
            ...
            if (!$this->getSession($mobo_service_id, $server_id)) {
                ...
                //Check Exist            
                if ($this->toploidai->check_exist_exchange_gift_top($tournament_id, $server_id, $char_id)) {
                    $result["code"] = "-1";
                    ...
                    //Check Receive Gift Date
                    $get_tournament_details = $this->toploidai->get_tournament_details($tournament_id);
                    $date_now = date('Y-m-d H:i:s');
                    ...
                    if (strtotime($date_now) > strtotime($tournament_date_end_reward)) {
                        ...
                        $server_id_merge = $this->get_server_id_merge($server_id);
                        $userpoint = $this->toploidai->get_pk_point_new($get_tournament_details[0]["week_no"], $server_id_merge, $char_id);
...
//                            if ($char_id == "1900000126") {
//                                $userpoint[0]["RANK"] = 5;
//                            }                            
                            $reward_rank_valid = $this->toploidai->check_rank_valid($userpoint[0]["TOP"], $tournament_id);

                            //Check NL Bonus
                            if ($reward_rank_valid[0]["reward_percent"] > 0) {                                
                                 if ($this->toploidai->check_exist_exchange_gift_top_nl($tournament_id, $server_id, $char_id)) {
                                    $result["code"] = "-1";
                                    $result["message"] = "Bạn đã nhận quà Top giải đấu này rồi !";
                                    $this->output->set_output(json_encode($result));
                                    return;
                                ...
                                $api = new GameFullAPI();
                                $server_list = preg_replace('/\s+/', '', $get_tournament_details[0]["tournament_server_list"]);
                                $server_list = explode(";", $server_list);
                                $getmoney_api = $api->get_money($this->service_name, NULL, $server_list, $get_tournament_details[0]["tournament_date_start"], $get_tournament_details[0]["tournament_date_end"], 2);
                                $percent_server = ($getmoney_api["amount"] / 100) * $get_tournament_details[0]["reward_percent"];
                                $point = ($percent_server / 100) * $reward_rank_valid[0]["reward_percent"];
                                $point = ceil($point / 100);

                                //Add NL
                                $datauser = $this->toploidai->user_check_point_exist($char_id, $server_id, $mobo_service_id);
                                if (count($datauser) > 0) {
                                    if ($this->toploidai->add_point($char_id, $server_id, $mobo_service_id, $point) > 0) {
                                        foreach ($datauser as $key => $u_value) {
                                            $userdata_p["user_id"] = $u_value["id"];
                                            $userdata_p["ex_type"] = "top_loi_dai";
                                            $userdata_p["ex_value"] = $point;
                                            $userdata_p["ex_date"] = Date('Y-m-d H:i:s');
                                            $userdata_p["tournament_id"] = $tournament_id;
                                            $userdata_p["reward_top_id"] = $reward_rank_valid[0]["id"];
                                            $this->toploidai->insert("event_toploidai_add_nganluong_history", $userdata_p);
                                        }
                                        ...
                                        $result["code"] = "-1";
                                        $result["message"] = "Nhận quà Top thất bại!";
                                        $this->output->set_output(json_encode($result));
                                        return;
                                    }
                                } else {
                                ...
                                if (count($reward_rank_valid) == 0) {
                                    ...
                                    $i_id = $this->toploidai->insert_id("event_toppk_exchange_history_top", $userdata_p);
                                    ...
                                    if ($i_id > 0) {
                                        ...
                                        $this->toploidai->update_exchange_history_top($i_id, json_encode($item1), json_encode($data_result));
    ...
    public function gift_top_exchange_premiership() {
        if (isset($_SESSION["execute_time"]) && (time() - $_SESSION["execute_time"]) < 10) {
            ...
            //}
            //else{
            $reward_details = $this->toploidai->get_reward_details_premiership($tournament_id);
            if (count($reward_details) > 0) {
                if (!$this->getSession($mobo_service_id, $server_id)) {
                    $result["code"] = "-1";
                    $result["message"] = "Bạn đang đăng nhập trên một máy khác.";
                    ...
                    //Check Exist            
                    if ($this->toploidai->check_exist_exchange_gift_premiership($tournament_id, $server_id, $char_id)) {
                        $result["code"] = "-1";
...
//                                $userpoint = $this->toploidai->get_pk_point_new($get_tournament_details[0]["tournament_store_proc"], $server_id_merge, $char_id);
//                            } else {
//                                $userpoint = $this->toploidai->get_pk_point_new($get_tournament_details[0]["tournament_store_proc"], $server_id, $char_id);
//                            }
                            ...
                            if (!($server_id == 2 && $char_id == "750000045") && count($userpoint) == 0) {
                                ...
                                if (!($server_id == 2 && $char_id == "750000045") && $userpoint[0]["CURRPOINT"] == 0) {
                                    ...
                                    $data_top_premiership = $this->toploidai->get_top($get_tournament_details[0]["tournament_store_proc_top_premiership"]);
                                    ...
                                    foreach ($data_top_premiership as $key => $value) {
                                        if ($char_id == $value['UID']) {
                                            ...
                                            $i_id = $this->toploidai->insert_id("event_toppk_exchange_history_premiership", $userdata_p);
                                            ...
                                            if ($i_id > 0) {
                                                ...
                                                $this->toploidai->update_exchange_history_premiership($i_id, json_encode($item1), json_encode($data_result));
...
//        if (in_array($server_id, $this->server_merge)) {
//            $server_id_merge = 3;
//            $userpoint = $this->toploidai->get_pk_point_new($tournament_list[0]["tournament_store_proc"], $server_id_merge, $char_id);
//        } else
//        if (in_array($server_id, $this->server_merge2)) {
//            $server_id_merge = 1;
//            $userpoint = $this->toploidai->get_pk_point_new($tournament_list[0]["tournament_store_proc"], $server_id_merge, $char_id);
...
//            $userpoint = $this->toploidai->get_pk_point_new($tournament_list[0]["tournament_store_proc"], $server_id_merge, $char_id);
//        } else {
//            $userpoint = $this->toploidai->get_pk_point_new($tournament_list[0]["tournament_store_proc"], $server_id, $char_id);
//        }
        ...
        //Get TOP
        if ($server_id_s == 0) {
            $data["get_top"] = $this->toploidai->get_top($tournament_list[0]["tournament_store_proc_top"]);
            $this->data["get_top"] = $data["get_top"];
        } else {
            $data["get_top_server"] = $this->toploidai->get_top_server($tournament_list[0]["tournament_store_proc_top_server"], $server_id_s);
            $this->data["get_top_server"] = $data["get_top_server"];
        }

        $this->output->set_header("Cache-Control: no-store, no-cache, must-revalidate");
        echo $this->load->view("events/toploidai/top", $this->data, true);
    }
...
//        }
        ...
        //Get TOP
        if ($id >= 12) {
            $data["data_top_premiership"] = $this->toploidai->get_top($tournament_list[0]["tournament_store_proc_top_premiership"]);
        ...
        $data["data_reward_premiership"] = $this->toploidai->get_reward_details_premiership($id);
        $this->data["data_reward_premiership"] = $data["data_reward_premiership"];

        $this->output->set_header("Cache-Control: no-store, no-cache, must-revalidate");
        echo $this->load->view("events/toploidai/top_premiership", $this->data, true);
    }
    ...
    function get_top_allserver() {
        ...
        //Get TOP
        if ($id >= 14) {
            $data["data_top_all_server"] = $this->toploidai->get_top($tournament_list[0]["tournament_store_proc_top_all_server"]);
        ...
        $this->output->set_header("Cache-Control: no-store, no-cache, must-revalidate");
        echo $this->load->view("events/toploidai/top_all_server", $this->data, true);
    }
    ...
    function tournament_list_ex() {
        ...
        $tournament_list = $this->toploidai->tournament_list();
    ...
    function get_top_arena() {
        ...
        //Test
//        $api = new GameFullAPI();
//        $server_list = preg_replace('/\s+/', '', $tournament_list[0]["tournament_server_list"]);
//        $server_list = explode(";", $server_list);             
//        $getmoney_api = $api->get_money($this->service_name, NULL, $server_list, $tournament_list[0]["tournament_date_start"], $tournament_list[0]["tournament_date_end"], 2);
//        var_dump($getmoney_api); die;
        //Get TOP      
        $this->data["tournament_id"] = $id;
        $this->data["TopArena"] = $this->toploidai->Event_TopArena_GetList((int) $tournament_list[0]["week_no"]);
        
        //Get Details
        $server_id_merge = $this->get_server_id_merge($server_id);
        $userpoint = $this->toploidai->get_pk_point_new($tournament_list[0]["week_no"], $server_id_merge, $char_id);
        $this->data["userpoint"] = $userpoint;
        //var_dump( $this->data["TopArena"]); die;
    ...
    function get_top_battle() {
        ...
        //Get TOP      
        $this->data["tournament_id"] = $id;
        $this->data["TopBattle"] = $this->toploidai->Event_TopBattlePoint_GetList($server_id, (int) $tournament_list[0]["week_no"]);

        $this->output->set_header("Cache-Control: no-store, no-cache, must-revalidate");
        echo $this->load->view("events/toploidai/top_battle", $this->data, true);
    }
    ...
    public function get_top_percent($reward_rank, $tournament_id) {
        ...
        $reward_details_top = $this->toploidai->check_rank_valid($reward_rank, $tournament_id);
        //var_dump($reward_details_top); die;

        if (count($reward_details_top) > 0 && $reward_details_top[0]["reward_percent"] > 0) {
            $tournament_details = $this->toploidai->get_tournament_details($tournament_id);
            $server_list = preg_replace('/\s+/', '', $tournament_details[0]["tournament_server_list"]);
            $server_list = explode(";", $server_list);

            if (count($reward_details_top) > 0) {
                $api = new GameFullAPI();
                $getmoney_api = $api->get_money($this->service_name, NULL, $server_list, $tournament_details[0]["tournament_date_start"], $tournament_details[0]["tournament_date_end"], 2);
    ...
    public function get_premier_percent($reward_rank, $tournament_id) {
        ...
        $reward_details_top = $this->toploidai->check_rank_premier_valid($reward_rank, $tournament_id);
        //var_dump($reward_details_top); die;
    ...
    private function user_point($char_id, $server_id, $mobo_service_id, $mobo_id, $char_name) {
        ...
        $user_check_point_exist_mobo = $this->toploidai->user_check_point_exist_mobo($mobo_id);
        if (count($user_check_point_exist_mobo) > 1) {
            $update_moboid_null = $this->toploidai->update_moboid_null($mobo_id);
        }
        ...
        if (count($datauser) > 0) {
            foreach ($datauser as $key => $value) {
                ...
                if ($value["mobo_id"] == null || empty($value["mobo_id"]) || ($value["mobo_id"] != $mobo_id)) {
                    $this->toploidai->update_shopnganluong_point_moboid($value["id"], $mobo_id);
                    ...
                    $this->toploidai->insert("event_shopnganluong_point_update_moboid_history", $userdata_p);
                }
            ...
            $this->toploidai->insert("event_shopnganluong_point", $userdata_p);
