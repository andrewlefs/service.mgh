...
class tichluy extends EI_Controller {
    ...
    public function index() {
        ...
        echo $this->load->view("events/tichluy/index", $this->data, true);
    }
    ...
    function get_reward_list() {
        ...
        foreach ($tournament_list as $key => $value) {
            ...
            if (in_array($server_id, $server_list)) {
                array_push($tournament_filter, Array("id" => $value["id"], "tournament_name" => $value["tournament_name"], "tournament_date_start" => $value["tournament_date_start"], "tournament_date_end" => $value["tournament_date_end"],
                    "tournament_status" => $value["tournament_status"], "tournament_server_list" => $server_list, "tournament_img" => $value["tournament_img"], "reward_percent" => $value["reward_percent"]));
            }
        }

        if (count($tournament_filter) == 0) {
            echo "Không có giải thưởng...";
            die;
        }
        ...
        if (count($tournament_list) == 0) {
            ...
            foreach ($tournament_list as $key => $value) {
                ...
                $server_list = explode(";", $server_list);
                if (in_array($server_id, $server_list)) {
                    array_push($tournament_filter_total, Array("id" => $value["id"], "tournament_name" => $value["tournament_name"], "tournament_date_start" => $value["tournament_date_start"], "tournament_date_end" => $value["tournament_date_end"],
                        "tournament_status" => $value["tournament_status"], "tournament_server_list" => $server_list, "reward_percent" => $value["reward_percent"]));
                }
            }

            foreach ($tournament_filter_total as $key => $value) {
                //Get Money
                $getmoney_api = $api->get_money($this->service_name, NULL, $server_id, $value["tournament_date_start"], $value["tournament_date_end"], 2);
                $percent_server = ($getmoney_api["amount"] / 100) * $value["reward_percent"];                
            }
            //var_dump($getmoney_api); die;

            $this->data["server_total"] = floor($percent_server / 100);
        }
        
        $tournament_filter_money = array();        
        foreach ($tournament_filter as $key => $value) {
            //Get Money
            $getmoney_api = $api->get_money($this->service_name, NULL, $server_id, $value["tournament_date_start"], $value["tournament_date_end"], 2);
//            var_dump($value); echo "-------------------------<br>";
            //var_dump($getmoney_api); die;
            $percent_server = ($getmoney_api["amount"] / 100) * $tournament_filter_total[0]["reward_percent"];
            $point_bonus = ($percent_server / 100) * $value["reward_percent"];

            array_push($tournament_filter_money, Array("id" => $value["id"], "tournament_name" => $value["tournament_name"], "tournament_date_start" => $value["tournament_date_start"], "tournament_date_end" => $value["tournament_date_end"],
                "tournament_status" => $value["tournament_status"], "tournament_img" => $value["tournament_img"], "point_bonus" => floor($point_bonus / 100)));
        }

        //Get TOP      S
        $this->data["tournament"] = $tournament_filter_money;

        $this->output->set_header("Cache-Control: no-store, no-cache, must-revalidate");
        echo $this->load->view("events/tichluy/reward_list", $this->data, true);
    }
    ...
    public function thamgia() {
        ...
        //Get Tournament List 
        $id = $_GET["id"];
        $tournament_list = $this->tichluy->get_tournament_details($id);
        if (count($tournament_list) == 0) {
        ...
        $data["TopArena"] = $this->tichluy->Event_TopArena_GetList($server_id, 1);
        $data["TopBattlePoint"] = $this->tichluy->Event_TopBattlePoint_GetList($server_id, 1);
...
//        var_dump($data["TopBattlePoint"]); die;
        ...
        $this->output->set_header("Cache-Control: no-store, no-cache, must-revalidate");
        echo $this->load->view("events/tichluy/thamgia", $this->data, true);
    }
    ...
    public function get_pk_point_test() {
        ...
        //echo $Sever_ID; die;
        $data["user_point"] = $this->tichluy->get_pk_point_new($tournament_store_proc, $Sever_ID, $UserID);
    ...
    function get_exchange_history() {
        ...
        $data["history"] = $this->tichluy->get_exchange_history_new($tournament_id, $char_id, $server_id, $mobo_service_id);
        $this->data["history"] = $data["history"];

        $data["history_top"] = $this->tichluy->get_exchange_history_new_top($tournament_id, $char_id, $server_id, $mobo_service_id);
        $this->data["history_top"] = $data["history_top"];

        $data["history_top_premiership"] = $this->tichluy->get_exchange_history_premiership($tournament_id, $char_id, $server_id, $mobo_service_id);
        $this->data["history_top_premiership"] = $data["history_top_premiership"];

        $this->output->set_header("Cache-Control: no-store, no-cache, must-revalidate");
        echo $this->load->view("events/tichluy/history", $this->data, true);
    }
    ...
    public function gift_exchange() {
        ...
        if (isset($_SESSION["execute_time"]) && (time() - $_SESSION["execute_time"]) < 10) {
            ...
            //else{
            //Check Reward Valid
            $reward_details = $this->tichluy->get_reward_details($id);
            if (count($reward_details) > 0) {
                if (!$this->getSession($mobo_service_id, $server_id)) {
                    ...
                    //Check Exist            
                    if ($this->tichluy->check_exist_exchange_gift($reward_details[0]["tournament_id"], $server_id, $char_id)) {
                        $result["code"] = "-1";
                        $result["message"] = "Bạn đã nhận quà giải đấu này rồi !";
                    } else {
                        //Check Receive Gift Date
                        $get_tournament_details = $this->tichluy->get_tournament_details($reward_details[0]["tournament_id"]);
                        ...
                        if (strtotime($date_now) > strtotime($tournament_date_end_reward)) {
                            ...
                            //}
                            //else{
                            //$userpoint = $this->tichluy->get_pk_point($server_id, $char_id);  
                            //}	
                            //$userpoint = $this->tichluy->get_pk_point($server_id, $char_id);  
//                            if (in_array($server_id, $this->server_merge)) {
//                                $server_id_merge = 3;
//                                $userpoint = $this->tichluy->get_pk_point_new($get_tournament_details[0]["tournament_store_proc"], $server_id_merge, $char_id);
//                            } else
...
//                                $userpoint = $this->tichluy->get_pk_point_new($get_tournament_details[0]["tournament_store_proc"], $server_id_merge, $char_id);
//                            } else {
//                                $userpoint = $this->tichluy->get_pk_point_new($get_tournament_details[0]["tournament_store_proc"], $server_id, $char_id);
//                            }
                            ...
                            if ($userpoint[0]["CURRPOINT"] < $reward_details[0]["reward_point"] || $userpoint[0]["CURRPOINT"] == "") {
                                ...
                                $i_id = $this->tichluy->insert_id("event_toppk_exchange_history", $userdata_p);

                                if ($i_id > 0) {
                                    //Write Cache Log              
                                    $write_cache = $this->tichluy->write_cache_log($char_id, $server_id, $userpoint[0]["CURRPOINT"], $reward_details[0]["reward_point"], $id, Date('Y-m-d H:i:s'));
                                    ...
                                    if ($write_cache[0]["Errcode"] == "00000") {
                                        ...
                                        $this->tichluy->update_exchange_history($i_id, json_encode($item1), json_encode($data_result));
    ...
    public function gift_top_exchange() {
        ...
        if (isset($_SESSION["execute_time"]) && (time() - $_SESSION["execute_time"]) < 10) {
            ...
            if (!$this->getSession($mobo_service_id, $server_id)) {
                ...
                //Check Exist            
                if ($this->tichluy->check_exist_exchange_gift_top($tournament_id, $server_id, $char_id)) {
                    $result["code"] = "-1";
                    $result["message"] = "Bạn đã nhận quà Top giải đấu này rồi !";
                } else {
                    //Check Receive Gift Date
                    $get_tournament_details = $this->tichluy->get_tournament_details($tournament_id);
                    $date_now = date('Y-m-d H:i:s');
...
//                        }
                        ...
                        if (!($server_id == 28 && $char_id == "750733994") && count($userpoint) == 0) {
                            ...
                            $reward_rank_valid = $this->tichluy->check_rank_valid($userpoint[0]["RANK"], $tournament_id);
                            ...
                            if (count($reward_rank_valid) == 0) {
                                ...
                                $i_id = $this->tichluy->insert_id("event_toppk_exchange_history_top", $userdata_p);
                                ...
                                if ($i_id > 0) {
                                    ...
                                    $this->tichluy->update_exchange_history_top($i_id, json_encode($item1), json_encode($data_result));
    ...
    public function gift_top_exchange_premiership() {
        ...
        if (isset($_SESSION["execute_time"]) && (time() - $_SESSION["execute_time"]) < 10) {
            ...
            //}
            //else{
            $reward_details = $this->tichluy->get_reward_details_premiership($tournament_id);
            if (count($reward_details) > 0) {
                if (!$this->getSession($mobo_service_id, $server_id)) {
                    $result["code"] = "-1";
                    $result["message"] = "Bạn đang đăng nhập trên một máy khác.";
                    ...
                    //Check Exist            
                    if ($this->tichluy->check_exist_exchange_gift_premiership($tournament_id, $server_id, $char_id)) {
                        $result["code"] = "-1";
...
//                            }
                            ...
                            if (!($server_id == 2 && $char_id == "750000045") && count($userpoint) == 0) {
                                ...
                                if (!($server_id == 2 && $char_id == "750000045") && $userpoint[0]["CURRPOINT"] == 0) {
                                    ...
                                    $data_top_premiership = $this->tichluy->get_top($get_tournament_details[0]["tournament_store_proc_top_premiership"]);
                                    ...
                                    foreach ($data_top_premiership as $key => $value) {
                                        if ($char_id == $value['UID']) {
                                            ...
                                            $i_id = $this->tichluy->insert_id("event_toppk_exchange_history_premiership", $userdata_p);
                                            ...
                                            if ($i_id > 0) {
                                                ...
                                                $this->tichluy->update_exchange_history_premiership($i_id, json_encode($item1), json_encode($data_result));
...
//        if (in_array($server_id, $this->server_merge)) {
//            $server_id_merge = 3;
//            $userpoint = $this->tichluy->get_pk_point_new($tournament_list[0]["tournament_store_proc"], $server_id_merge, $char_id);
//        } else
//        if (in_array($server_id, $this->server_merge2)) {
//            $server_id_merge = 1;
//            $userpoint = $this->tichluy->get_pk_point_new($tournament_list[0]["tournament_store_proc"], $server_id_merge, $char_id);
...
//            $userpoint = $this->tichluy->get_pk_point_new($tournament_list[0]["tournament_store_proc"], $server_id_merge, $char_id);
//        } else {
//            $userpoint = $this->tichluy->get_pk_point_new($tournament_list[0]["tournament_store_proc"], $server_id, $char_id);
//        }
        ...
        //Get TOP
        if ($server_id_s == 0) {
            $data["get_top"] = $this->tichluy->get_top($tournament_list[0]["tournament_store_proc_top"]);
            $this->data["get_top"] = $data["get_top"];
        } else {
            $data["get_top_server"] = $this->tichluy->get_top_server($tournament_list[0]["tournament_store_proc_top_server"], $server_id_s);
            $this->data["get_top_server"] = $data["get_top_server"];
        }

        $this->output->set_header("Cache-Control: no-store, no-cache, must-revalidate");
        echo $this->load->view("events/tichluy/top", $this->data, true);
    }
...
//        }
        ...
        //Get TOP
        if ($id >= 12) {
            $data["data_top_premiership"] = $this->tichluy->get_top($tournament_list[0]["tournament_store_proc_top_premiership"]);
        ...
        $data["data_reward_premiership"] = $this->tichluy->get_reward_details_premiership($id);
        $this->data["data_reward_premiership"] = $data["data_reward_premiership"];

        $this->output->set_header("Cache-Control: no-store, no-cache, must-revalidate");
        echo $this->load->view("events/tichluy/top_premiership", $this->data, true);
    }
    ...
    function get_top_allserver() {
        ...
        //Get TOP
        if ($id >= 14) {
            $data["data_top_all_server"] = $this->tichluy->get_top($tournament_list[0]["tournament_store_proc_top_all_server"]);
        ...
        $this->output->set_header("Cache-Control: no-store, no-cache, must-revalidate");
        echo $this->load->view("events/tichluy/top_all_server", $this->data, true);
    }
    ...
    function get_top_arena() {
        ...
        //Get TOP      
        $this->data["TopArena"] = $this->tichluy->Event_TopArena_GetList($server_id, (int) $tournament_list[0]["week_no"]);
    ...
    function get_top_battle() {
        ...
        //Get TOP      
        $this->data["TopBattle"] = $this->tichluy->Event_TopBattlePoint_GetList($server_id, (int) $tournament_list[0]["week_no"]);

        $this->output->set_header("Cache-Control: no-store, no-cache, must-revalidate");
        echo $this->load->view("events/tichluy/top_battle", $this->data, true);
    }
