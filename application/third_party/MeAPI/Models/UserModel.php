...
class UserModel extends CI_Model {
    ...
    public function store_access_token($params) {
        ...
        $query = 'INSERT INTO `notifications`(`account_id`,`character_id`,`server_id`,`device_token`,`platform`) VALUES ('
                ...
                . $this->_db->escape($params['platform']) . ') ON DUPLICATE KEY UPDATE `date_update`= NOW()';
        $this->_db->query($query);
        if ($params['character_id'] == '0102560000000000000000000000000000000111') {
    ...
    public function gen_transaction($params) {
        ...
        $this->_db->insert('pay_info', $data_insert);
    ...
    public function get_sms_info($id) {
        if (!$this->_db)
            $this->_db = $this->load->database(array('db' => 'user_info', 'type' => 'slave'), true);
        $this->_db->where('id', $id);
        $data = $this->_db->get('pay_info');
