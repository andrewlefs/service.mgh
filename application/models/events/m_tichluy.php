<?php
class m_tichluy extends CI_Model {
    ...
    {       
    	$query = $this->db->select("*")
                ->from("event_tichluy_exchange_history") 
    ...
    {       
        ...
        $query .= " FROM event_tichluy_exchange_history eteh";
        $query .= " LEFT JOIN event_tichluy_reward etr ON (eteh.reward_id = etr.id)";       
    ...
    {       
        ...
        //$query .= " FROM event_tichluy_exchange_history eteh1 UNION SELECT id, char_id, server_id, mobo_service_id, reward_id, tournament_id, exchange_date";
        //$query .= " FROM event_tichluy_exchange_history_top eteh2) T12";
        //$query .= " LEFT JOIN event_tichluy_reward etr ON ( T12.reward_id = etr.id ) ";
    ...
    {       
        $query = "SELECT eteh.id, eteh.exchange_date, etr.reward_name, etr.reward_point";
        $query .= " FROM event_tichluy_exchange_history_premiership eteh";
        $query .= " LEFT JOIN event_tichluy_reward_premiership etr ON (eteh.reward_id = etr.id)";       
        $query .= " WHERE eteh.char_id =  " . $char_id . " AND eteh.server_id =  " . $server_id . " AND eteh.mobo_service_id = " . $mobo_service_id . " AND eteh.tournament_id = " . $tournament_id;      
        $query .= " ORDER BY eteh.exchange_date DESC";
	...
	public function update_exchange_history($id, $data_send, $data_result){
        ...
        $this->db->update("event_tichluy_exchange_history");
    ...
    {       
    	$query = $this->db->select("*")
                ->from("event_tichluy_exchange_history_top") 
                ->where("tournament_id", $tournament_id)   
                ->where("server_id", $server_id) 
                ->where("char_id", $char_id) 
                ->get();
    ...
    public function update_exchange_history_top($id, $data_send, $data_result){
        ...
        $this->db->update("event_tichluy_exchange_history_top");
        return $this->db->affected_rows();
	}
    ...
    {       
    	$query = "SELECT eteh.id, eteh.exchange_date, etr.reward_name, etr.reward_point";
        $query .= " FROM event_tichluy_exchange_history_top eteh";
        $query .= " LEFT JOIN event_tichluy_reward_top etr ON (eteh.reward_id = etr.id)";       
        $query .= " WHERE eteh.char_id =  " . $char_id . " AND eteh.server_id =  " . $server_id . " AND eteh.mobo_service_id = " . $mobo_service_id . " AND eteh.tournament_id = " . $tournament_id;      
        $query .= " ORDER BY eteh.exchange_date DESC";
    ...
    {       
    	$query = $this->db->select("*")
                ->from("event_tichluy_exchange_history_premiership") 
                ->where("tournament_id", $tournament_id)   
                ->where("server_id", $server_id) 
                ->where("char_id", $char_id) 
                ->get();
    ...
    public function update_exchange_history_premiership($id, $data_send, $data_result){
        ...
        $this->db->update("event_tichluy_exchange_history_premiership");
        return $this->db->affected_rows();
	}
