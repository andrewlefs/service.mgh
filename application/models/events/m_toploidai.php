<?php
class m_toploidai extends CI_Model {
    ...
    {       
    	$query = $this->db->select("*")
                ->from("event_toploidai_exchange_history") 
    ...
    {       
        ...
        $query .= " FROM event_toploidai_exchange_history eteh";
        $query .= " LEFT JOIN event_toploidai_reward etr ON (eteh.reward_id = etr.id)";       
    ...
    {       
        ...
        //$query .= " FROM event_toploidai_exchange_history eteh1 UNION SELECT id, char_id, server_id, mobo_service_id, reward_id, tournament_id, exchange_date";
        //$query .= " FROM event_toploidai_exchange_history_top eteh2) T12";
        //$query .= " LEFT JOIN event_toploidai_reward etr ON ( T12.reward_id = etr.id ) ";
    ...
    {       
        $query = "SELECT eteh.id, eteh.exchange_date, etr.reward_name, etr.reward_point";
        $query .= " FROM event_toploidai_exchange_history_premiership eteh";
        $query .= " LEFT JOIN event_toploidai_reward_premiership etr ON (eteh.reward_id = etr.id)";       
        $query .= " WHERE eteh.char_id =  " . $char_id . " AND eteh.server_id =  " . $server_id . " AND eteh.mobo_service_id = " . $mobo_service_id . " AND eteh.tournament_id = " . $tournament_id;      
        $query .= " ORDER BY eteh.exchange_date DESC";
    ...
    public function get_pk_point_new($week_no, $Sever_ID, $UserID)
    {
        $this->freeDBResource($this->db_cache_mgh2->conn_id);
        $sql = "CALL event_loidai_get_point_userleitai(?,?,?)";       
        $parameters = array($week_no, $Sever_ID, $UserID);
        $query = $this->db_cache_mgh2->query($sql, $parameters);
	...
	public function update_exchange_history($id, $data_send, $data_result){
        ...
        $this->db->update("event_toploidai_exchange_history");
        return $this->db->affected_rows();
	}
    
    //Reward Top
    public function check_exist_exchange_gift_top_nl($tournament_id, $server_id, $char_id)
    {       
    	$query = $this->db->select("*")
                ->from("event_toploidai_add_nganluong_history") 
                ->where("tournament_id", $tournament_id)   
                ->where("server_id", $server_id) 
                ->where("char_id", $char_id) 
                ->get();
    ...
    {       
    	$query = $this->db->select("*")
                ->from("event_toploidai_exchange_history_top") 
                ->where("tournament_id", $tournament_id)   
                ->where("server_id", $server_id) 
                ->where("char_id", $char_id) 
                ->get();
    ...
    public function update_exchange_history_top($id, $data_send, $data_result){
        ...
        $this->db->update("event_toploidai_exchange_history_top");
        return $this->db->affected_rows();
	}
    ...
    {       
    	$query = "SELECT eteh.id, eteh.exchange_date, etr.reward_name, etr.reward_point";
        $query .= " FROM event_toploidai_exchange_history_top eteh";
        $query .= " LEFT JOIN event_toploidai_reward_top etr ON (eteh.reward_id = etr.id)";       
        $query .= " WHERE eteh.char_id =  " . $char_id . " AND eteh.server_id =  " . $server_id . " AND eteh.mobo_service_id = " . $mobo_service_id . " AND eteh.tournament_id = " . $tournament_id;      
        $query .= " ORDER BY eteh.exchange_date DESC";
    ...
    {       
    	$query = $this->db->select("*")
                ->from("event_toploidai_exchange_history_premiership") 
                ->where("tournament_id", $tournament_id)   
                ->where("server_id", $server_id) 
                ->where("char_id", $char_id) 
                ->get();
    ...
    public function update_exchange_history_premiership($id, $data_send, $data_result){
        ...
        $this->db->update("event_toploidai_exchange_history_premiership");
        return $this->db->affected_rows();
	}
        
    //Get Top New
    public function Event_TopArena_GetList($week_event){
        $this->freeDBResource($this->db_cache_mgh2->conn_id);
        $sql = "CALL event_loidai_get_top_userleitai($week_event)"; 
    ...
    public function Event_TopBattlePoint_GetList($server_id, $week_event){
        $this->freeDBResource($this->db_cache_mgh2->conn_id);
        $sql = "CALL Event_TopBattlePoint_GetList('$server_id', $week_event)";       
        $query = $this->db_cache_mgh2->query($sql);
