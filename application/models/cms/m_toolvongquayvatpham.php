...
class m_toolvongquayvatpham extends Backend_Model {
    private $db;
    ...
    function add_tournament($params) {
        ...
        $this->db->insert('event_vongquayvatpham_tournament', $params);
    ...
    function edit_tournament($params) {
        ...
        $this->db->update("event_vongquayvatpham_tournament");
        return $this->db->affected_rows();
    }

    function tournament_list() {
        $this->datatables_config = array(
            "table" => "event_vongquayvatpham_tournament",
            "select" => "
    ...
    function get_vongquayvatpham_gift($tournament_id) {
        $where = "WHERE tournament_id = $tournament_id";
        $this->datatables_config = array(
            "table" => "event_vongquayvatpham_gift",
            "select" => "
                    SELECT SQL_CALC_FOUND_ROWS *
    ...
    function add_vongquayvatpham_gift($params) {
        if (empty($params)) {
            return 0;
        }

        $this->db->insert('event_vongquayvatpham_gift', $params);
        return $this->db->affected_rows();
    }
    ...
    function edit_vongquayvatpham_gift($params) {
        if (empty($params) || empty($params['id'])) {
            return 0;
        }
        ...
        $this->db
                ...
                ->set('percentage', $params['percentage'])
                ->set('item_img', $params['item_img'])
                ->where("id", $params['id']);

        $this->db->update("event_vongquayvatpham_gift");
    ...
    function get_exchange_history($tournament_id, $startdate, $enddate) {
        ...
        $this->datatables_config = array(
            "table" => "event_vongquayvatpham_history",
            "select" => "
                    SELECT SQL_CALC_FOUND_ROWS *
                    FROM event_vongquayvatpham_history",
            "where" => $where,
            "order_by" => " ORDER BY id DESC",
            "columnmaps" => array(
            )
...
//        $query .= " FROM event_vongquayvatpham_exchange_history eteh LEFT JOIN event_vongquayvatpham_gift_pakage etr ON (eteh.pakage_id = etr.id)";
