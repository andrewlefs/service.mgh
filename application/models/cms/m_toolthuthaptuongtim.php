...
class m_toolthuthaptuongtim extends Backend_Model {
    private $db;
    ...
    function add_tournament($params) {
        ...
        $this->db->insert('event_thuthaptuongtim_tournament', $params);
    ...
    function edit_tournament($params) {
        ...
        $this->db->update("event_thuthaptuongtim_tournament");
        return $this->db->affected_rows();
    }

    function tournament_list() {
        $this->datatables_config = array(
            "table" => "event_thuthaptuongtim_tournament",
            "select" => "
    ...
    function get_thuthaptuongtim_gift($tournament_id) {
        $where = "WHERE tournament_id = $tournament_id";
        $this->datatables_config = array(
            "table" => "event_thuthaptuongtim_gift",
            "select" => "
                    SELECT SQL_CALC_FOUND_ROWS *
    ...
    function add_thuthaptuongtim_gift($params) {
        if (empty($params)) {
            return 0;
        }

        $this->db->insert('event_thuthaptuongtim_gift', $params);
        return $this->db->affected_rows();
    }
    ...
    function edit_thuthaptuongtim_gift($params) {
        if (empty($params) || empty($params['id'])) {
            return 0;
        }
        ...
        $this->db->update("event_thuthaptuongtim_gift");
    ...
    function get_exchange_history($tournament_id, $startdate, $enddate) {
        ...
        $this->datatables_config = array(
            "table" => "event_thuthaptuongtim_history",
            "select" => "
                    SELECT SQL_CALC_FOUND_ROWS *
                    FROM event_thuthaptuongtim_history",
            "where" => $where,
            "order_by" => " ORDER BY id DESC",
            "columnmaps" => array(
            )
...
//        $query .= " FROM event_thuthaptuongtim_exchange_history eteh LEFT JOIN event_thuthaptuongtim_gift_pakage etr ON (eteh.pakage_id = etr.id)";
