...
class m_toolduatopdautruong extends Backend_Model {
    private $db;
    ...
    function add_tournament($params) {
        ...
        $this->db->insert('event_duatopdautruong_tournament', $params);
    ...
    function edit_tournament($params) {
        ...
        $this->db->update("event_duatopdautruong_tournament");
        return $this->db->affected_rows();
    }
    ...
    function tournament_list() {
        $this->datatables_config = array(
            "table" => "event_duatopdautruong_tournament",
            "select" => "
    ...
    function get_duatopdautruong_gift($tournament_id) {
        $where = "WHERE tournament_id = $tournament_id";
        $this->datatables_config = array(
            "table" => "event_duatopdautruong_gift",
            "select" => "
                    SELECT SQL_CALC_FOUND_ROWS *
    ...
    function add_duatopdautruong_gift($params) {
        if (empty($params)) {
            return 0;
        }

        $this->db->insert('event_duatopdautruong_gift', $params);
        return $this->db->affected_rows();
    }
    ...
    function edit_duatopdautruong_gift($params) {
        if (empty($params) || empty($params['id'])) {
            return 0;
        }
        ...
        $this->db->update("event_duatopdautruong_gift");
        return $this->db->affected_rows();
    }
    ...
    function get_exchange_history($tournament_id, $startdate, $enddate) {
        ...
        $this->datatables_config = array(
            "table" => "event_duatopdautruong_history",
            "select" => "
                    SELECT SQL_CALC_FOUND_ROWS *
                    FROM event_duatopdautruong_history",
            "where" => $where,
            "order_by" => " ORDER BY id DESC",
            "columnmaps" => array(
            )
...
//        $query .= " FROM event_duatopdautruong_exchange_history eteh LEFT JOIN event_duatopdautruong_gift_pakage etr ON (eteh.pakage_id = etr.id)";
