...
class m_tooldoivatpham extends Backend_Model {
    private $db;
    ...
    function add_tournament($params) {
        ...
        $this->db->insert('event_doivatpham_tournament', $params);
    ...
    function edit_tournament($params) {
        ...
        $this->db->update("event_doivatpham_tournament");
        return $this->db->affected_rows();
    }

    function tournament_list() {
        $this->datatables_config = array(
            "table" => "event_doivatpham_tournament",
            "select" => "
    ...
    function get_doivatpham_gift($tournament_id) {
        $where = "WHERE tournament_id = $tournament_id";
        $this->datatables_config = array(
            "table" => "event_doivatpham_gift",
            "select" => "
                    SELECT SQL_CALC_FOUND_ROWS *
    ...
    function add_doivatpham_gift($params) {
        if (empty($params)) {
            return 0;
        }

        $this->db->insert('event_doivatpham_gift', $params);
        return $this->db->affected_rows();
    }
    ...
    function edit_doivatpham_gift($params) {
        if (empty($params) || empty($params['id'])) {
            return 0;
        }
        ...
        $this->db
                ...
                ->set('gold_consumption', $params['gold_consumption'])
                ->set('gem_consumption', $params['gem_consumption'])
                ->set('times_per_day', $params['times_per_day'])
                ->set('item_img', $params['item_img'])
                ->where("id", $params['id']);

        $this->db->update("event_doivatpham_gift");
    ...
    function get_exchange_history($tournament_id, $startdate, $enddate) {
        ...
        $this->datatables_config = array(
            "table" => "event_doivatpham_history",
            "select" => "
                    SELECT SQL_CALC_FOUND_ROWS *
                    FROM event_doivatpham_history",
            "where" => $where,
            "order_by" => " ORDER BY id DESC",
            "columnmaps" => array(
            )
...
//        $query .= " FROM event_doivatpham_exchange_history eteh LEFT JOIN event_doivatpham_gift_pakage etr ON (eteh.pakage_id = etr.id)";
