...
class m_toolkhongdoitroichung extends Backend_Model {
    private $db;
    ...
    function add_tournament($params) {
        ...
        $this->db->insert('event_khongdoitroichung_tournament', $params);
    ...
    function edit_tournament($params) {
        ...
        $this->db->update("event_khongdoitroichung_tournament");
        return $this->db->affected_rows();
    }

    function tournament_list() {
        $this->datatables_config = array(
            "table" => "event_khongdoitroichung_tournament",
            "select" => "
                    SELECT SQL_CALC_FOUND_ROWS *
                    FROM event_khongdoitroichung_tournament",
    ...
    function get_khongdoitroichung_gift($tournament_id) {
        $where = "WHERE tournament_id = $tournament_id";
        $this->datatables_config = array(
            "table" => "event_khongdoitroichung_gift",
            "select" => "
                    SELECT SQL_CALC_FOUND_ROWS *
                    FROM event_khongdoitroichung_gift",
            "where" => $where,
            "order_by" => " Order By id DESC",
            "columnmaps" => array(
            )
    ...
    function add_khongdoitroichung_gift($params) {
        if (empty($params)) {
            return 0;
        }

        $this->db->insert('event_khongdoitroichung_gift', $params);
        return $this->db->affected_rows();
    }
    ...
    function edit_khongdoitroichung_gift($params) {
        if (empty($params) || empty($params['id'])) {
            return 0;
        }
        ...
        $this->db->update("event_khongdoitroichung_gift");
//        echo $this->db->last_query(); die;
    ...
    function get_exchange_history($tournament_id, $startdate, $enddate) {
        ...
        $this->datatables_config = array(
            "table" => "event_khongdoitroichung_history",
            "select" => "
                    SELECT SQL_CALC_FOUND_ROWS *
                    FROM event_khongdoitroichung_history",
            "where" => $where,
            "order_by" => " ORDER BY id DESC",
            "columnmaps" => array(
            )
...
//        $query .= " FROM event_khongdoitroichung_exchange_history eteh LEFT JOIN event_khongdoitroichung_gift_pakage etr ON (eteh.pakage_id = etr.id)";
