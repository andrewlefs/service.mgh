...
class m_toolnaplandau extends Backend_Model {
    private $db;
    ...
    function add_tournament($params) {
        ...
        $this->db->insert('event_naplandau_tournament', $params);
    ...
    function edit_tournament($params) {
        ...
        $this->db->update("event_naplandau_tournament");
        return $this->db->affected_rows();
    }
    ...
    function tournament_list() {
        $this->datatables_config = array(
            "table" => "event_naplandau_tournament",
            "select" => "
    ...
    function get_exchange_history($tournament_id, $startdate, $enddate) {
        ...
        $this->datatables_config = array(
            "table" => "event_naplandau_history",
            "select" => "
                    SELECT SQL_CALC_FOUND_ROWS *
                    FROM event_naplandau_history",
            "where" => $where,
            "order_by" => " ORDER BY id DESC",
            "columnmaps" => array(
            )
...
//        $query .= " FROM event_naplandau_exchange_history eteh LEFT JOIN event_naplandau_gift_pakage etr ON (eteh.pakage_id = etr.id)";
